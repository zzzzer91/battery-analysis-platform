...
func (s *LoginService) Do() (*jd.Response, error) {
	...
	user.LoginCount += 1
	err = dal.GetMongoService().UpdateUserLoginTimeAndCount(user)
	if err != nil {
		return nil, err
	}
...
func (s *LoginByCookieService) Do() (*jd.Response, error) {
	user, err := cache.GetRedisService().GetUserFromCache(s.UserName)
	if err != nil {
		return nil, err
	}
	if !user.CheckStatusOk() {
...
func (s *LogoutService) Do() (*jd.Response, error) {
	err := cache.GetRedisService().DeleteUserFromCache(s.UserName)
	if err != nil {
		return nil, err
	}
