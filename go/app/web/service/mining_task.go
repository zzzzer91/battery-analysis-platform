...
func (s *CreateMiningTaskService) Do() (*jd.Response, error) {
	...
	// 检查是否达到创建任务上限
	if !cache.GetRedisService().CheckTaskLimit(constant.RedisKeyMiningTaskWorkingIdSet, 1) {
	...
	err = cache.GetRedisService().AddWorkingTaskIdToSet(constant.RedisKeyMiningTaskWorkingIdSet, taskID)
	if err != nil {
		return nil, err
	}

	// 创建 mongo 记录
	data, err := dal.GetMongoService().CreateMiningTask(taskID, s.TaskName, s.DataComeFrom, dateRange)
	if err != nil {
		return nil, err
	}
...
func (s *GetMiningTaskListService) Do() (*jd.Response, error) {
	data, err := dal.GetMongoService().GetMiningTaskList()
	if err != nil {
		return nil, err
	}
...
func (s *GetMiningTaskDataService) Do() (*jd.Response, error) {
	data, err := dal.GetMongoService().GetMiningTaskData(s.Id)
	if err != nil {
		return nil, err
	}
	return jd.Build(jd.SUCCESS, "", data), nil
...
func (s *DeleteMiningTaskService) Do() (*jd.Response, error) {
	...
	_, err := producer.GetCeleryService().Delay(constant.CeleryTaskMiningStopComputeModel, s.Id)
	if err != nil {
		return nil, err
	}

	err = cache.GetRedisService().DelWorkingTaskIdFromSet(constant.RedisKeyMiningTaskWorkingIdSet, s.Id)
	if err != nil {
		return nil, err
	}

	err = dal.GetMongoService().DeleteMiningTask(s.Id)
	if err != nil {
		return nil, err
	}
